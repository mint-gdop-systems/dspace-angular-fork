...
import {
  ...
  SelectionAction,
} from 'src/themes/dars/app/item-page/edit-item-page/item-bitstreams/item-bitstreams.service';

@Component({
  selector: 'ds-item-edit-bitstream-bundle',
  styleUrls: [
    '../../../../../../../app/item-page/edit-item-page/item-bitstreams/item-bitstreams.component.scss',
    './item-edit-bitstream-bundle.component.scss',
  ],
...
export class ItemEditBitstreamBundleComponent implements OnInit, OnDestroy {
  ...
  protected initializePagination() {
    this.paginationOptions =
      this.itemBitstreamsService.getInitialBitstreamsPaginationOptions(
        this.bundleName,
      );

    this.currentPaginationOptions$ = new BehaviorSubject(
      this.paginationOptions,
    );
    ...
    this.subscriptions.push(
      this.bitstreamsRD$
        .pipe(
          ...
          tap(
            (bitstreamsRD) =>
              (this.bundleSize = bitstreamsRD.payload.totalElements),
          ),
          paginatedListToArray(),
        )
      ...
      this.bitstreamsRD$
        .pipe(
          ...
          switchMap((bitstreams) =>
            this.objectUpdatesService.getFieldUpdatesExclusive(
              this.bundleUrl,
              bitstreams,
            ),
          ),
      ...
      this.bitstreamsRD$
        .pipe(
          paginatedListToArray(),
          map((bitstreams) =>
            this.itemBitstreamsService.mapBitstreamsToTableEntries(bitstreams),
          ),
        )
  ...
  protected initializeSelectionActions() {
    this.subscriptions.push(
      this.itemBitstreamsService
        .getSelectionAction$()
        .subscribe((selectionAction) =>
          this.handleSelectionAction(selectionAction),
        ),
    );
  }
  ...
  handleSelectionAction(selectionAction: SelectionAction) {
    if (
      hasNoValue(selectionAction) ||
      selectionAction.selectedEntry.bundle !== this.bundle
    ) {
      return;
    }
  ...
  protected redirectToCurrentPage(bitstreamEntry: SelectedBitstreamTableEntry) {
    ...
    const selectedEntryPage = this.bundleIndexToPage(
      bitstreamEntry.currentPosition,
    );
   ...
   * @protected
   */
  protected redirectToOriginalPage(
    bitstreamEntry: SelectedBitstreamTableEntry,
  ) {
    const currentPage = this.getCurrentPage();
  ...
  dragStart() {
    ...
    this.paginationComponent.shouldShowBottomPager
      .pipe(
        take(1),
  ...
  drop(event: CdkDragDrop<any>) {
    ...
    if (
      hasValue(droppedOnElement) &&
      hasValue(droppedOnElement.textContent) &&
      droppedOnElement.classList.contains('page-link')
    ) {
    ...
    this.itemBitstreamsService.performBitstreamMoveRequest(
      this.bundle,
      fromIndex,
      toIndex,
      finish,
  ...
  select(event: UIEvent, bitstream: BitstreamTableEntry) {
    ...
    if (
      hasValue(selectedBitstream) &&
      selectedBitstream.bitstream === bitstream
    ) {
   ...
   * @protected
   */
  protected createBitstreamSelectionObject(
    ...
    const position = this.pageIndexToBundleIndex(
      pageIndex,
      this.getCurrentPage(),
    );
